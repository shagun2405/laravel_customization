...
<accordian :title="'{{ __('admin::app.catalog.products.variations') }}'" :active="false">
    <div slot="header">
        {{ __('admin::app.catalog.products.variations') }}
...
@push('scripts')
        ...
        <form class="account-table-content" method="POST" action="{{ route('admin.catalog.products.store') }}" data-vv-scope="add-variant-form" @submit.prevent="addVariant('add-variant-form')">
            ...
            <div class="page-content">
                <div class="form-container">
                    ...
                    <div v-for='(attribute, index) in super_attributes' class="form-group" :class="[errors.has('add-variant-form.' + attribute.code) ? 'has-error' : '']">
    ...
    <script type="text/x-template" id="variant-item-template">
        <tr>
            ...
            <td>
                <div class="form-group" :class="[errors.has(variantInputName + '[images][' + index + ']') ? 'has-error' : '']">
            ...
            <td>
                <div class="form-group" :class="[errors.has(variantInputName + '[price]') ? 'has-error' : '']">
                    <input type="text" v-validate="'required|decimal|min:0'" v-model="variant.price" :name="[variantInputName + '[price]']" class="control" data-vv-as="&quot;{{ __('admin::app.catalog.products.price') }}&quot;"/>
                    <span class="control-error" v-if="errors.has(variantInputName + '[price]')">@{{ errors.first(variantInputName + '[price]') }}</span>
                </div>
            </td>

            <td>
                <div class="form-group" :class="[errors.has(variantInputName + '[weight]') ? 'has-error' : '']">
                    <input type="text" v-validate="'required|decimal|min:0'" v-model="variant.weight"  :name="[variantInputName + '[weight]']" class="control" data-vv-as="&quot;{{ __('admin::app.catalog.products.weight') }}&quot;"/>
                        ...
                        if (result) {
                            ...
                            if (filteredVariants.length) {
                                ...
                                window.showAlert(
                                        `alert-error`,
                                        'Warning',
                                        "{{ __('admin::app.catalog.products.variant-already-exist-message') }}"
                                    );
                            } else {
                                var optionIds = [];
                                for (var key in this_this.variant) {
