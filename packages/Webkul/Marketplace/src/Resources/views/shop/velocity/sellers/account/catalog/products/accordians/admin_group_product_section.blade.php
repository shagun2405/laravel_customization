...
@php
    ...
    $groupProductsCollection = $product->product->grouped_products()->with('associated_product')->get() 
@endphp
@if($groupProductsCollection->count() > 0)
    @php
        $productGroupedProduct = $groupProductsCollection->filter(function($item , $key){
                                    $groupedProductSellerPrice = \Webkul\Marketplace\Models\GroupProductSellerPrice::where([['product_grouped_product_id','=',$item->id],['marketplace_seller_id','=',auth()->guard('customer')->user()->id]])->first();
                                    if($groupedProductSellerPrice != NULL){
                                        $item->seller_sell_price = $groupedProductSellerPrice->seller_sell_price;
                                    }else{
                                        $item->seller_sell_price = 0;
                                    }
                                    return $item;
                                });
    @endphp
...
<accordian :title="'{{ __('admin::app.catalog.products.videos') }}'" 
    :active="{{ ($product->product->type == "grouped" ? 'true' : 'false') }}">
    <div slot="header">
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th class="sort-order">{{ __('marketplace::app.shop.sellers.account.catalog.products.selelr_price') }}</th>
                        ...
                        <tbody>
                            <tr v-for='(groupedProduct, index) in grouped_products' 
                                ...
                                <td>
                                    ...
                                    <input type="hidden" 
                                        :name="[inputName(groupedProduct,index) + '[product_grouped_product_id]']" :value="groupedProduct.id"/>
                                </td>
                                ...
                                <td>
                                    <div class="control-group" 
                                        :class="[errors.has(inputName(groupedProduct,index) + '[seller_sell_price]') ? 'has-error' : '']">
                                        <input type="number" 
                                                v-validate="'required|min_value:0'" 
                                                :name="[inputName(groupedProduct,index) + '[seller_sell_price]']" 
                                                v-model="groupedProduct.seller_sell_price" 
                                                class="control" 
                                                data-vv-as="&quot;{{ __('admin::app.catalog.products.sort-order') }}&quot;"/>
                                        <span class="control-error" 
                                            v-if="errors.has(inputName(groupedProduct) + '[seller_sell_price]')">
                                            @{{ errors.first(inputName(groupedProduct) + '[seller_sell_price]') }}</span>
                                    </div>
                                </td>
        ...
        Vue.component('grouped-product-section', {
            ...
            data: function() {
                return {
                    ...
                    searched_results: [],
                    grouped_products: @json($productGroupedProduct)
                }
            },
            methods: {
