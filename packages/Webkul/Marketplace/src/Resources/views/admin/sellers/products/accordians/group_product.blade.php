@if($product->product->type == "grouped")
    ...
    $groupProductsCollection = $product->product
        ->grouped_products()
    ...
    @if ($groupProductsCollection->count() > 0)
        @php
            $productGroupedProduct = $groupProductsCollection->filter(function ($item, $key) {
                $groupedProductSellerPrice = \Webkul\Marketplace\Models\GroupProductSellerPrice::where([
                    ['product_grouped_product_id', '=', $item->id],
                    [
        ...
        @endphp
    @endif
    <accordian :title="'{{ __('marketplace::app.shop.sellers.account.catalog.products.grouped_product') }}'"
