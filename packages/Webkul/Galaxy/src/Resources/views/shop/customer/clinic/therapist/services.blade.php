@extends('galaxy::layouts.master')

@section('page_title')
    {{ __('galaxy::app.customer.clinic.therapist.services') }}
@stop

@section('content')               
    <therapist-services></therapist-services>       
@stop

@push('scripts')
    <script type="text/x-template" id="therapist-services-template">
        <div>
            <div class="min-h-full max-w-3xl mx-auto pt-8">
                <div class="rounded-lg bg-white overflow-hidden shadow mt-10 ml-20">
                    <h2 class="sr-only" id="profile-overview-title">Profile Overview</h2>
                    <div class="bg-white p-6">
                        <div class="sm:flex sm:items-center sm:justify-between">
                            <div class="sm:flex sm:space-x-5">
                                <div class="flex-shrink-0">
                                    <img class="mx-auto h-20 w-20 rounded-full" src="data:image/jpeg;base64," alt="">
                                </div>

                                <div class="mt-4 text-center sm:mt-0 sm:pt-1 sm:text-left">
                                    <p class="text-md font-medium text-gray-600">{{ __('galaxy::app.customer.clinic.services.choice-for-clinic-treatment')}}</p>
                                    <p class="text-xl font-bold text-gray-900 sm:text-2xl">Andreas Kviby</p>
                                    
                                    <p class="text-md mt-4 font-medium text-gray-400">
                                    {{ __('galaxy::app.customer.clinic.services.default-services-text')}} <span class="font-bold">{{ __('galaxy::app.customer.clinic.services.match-continue')}}</span>
                                    </p>

                                </div>
                            </div>
                        </div>

                        <div class="pt-6 pb-10">
                            <label id="listbox-label" class="block text-md font-medium font-bold text-gray-700"> {{ __('galaxy::app.customer.clinic.services.choose-treatment')}} </label>
                            <div class="mt-1 relative mb-4">
                                <select type="select" class="bg-white relative w-full border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-green-800 focus:border-green-800 sm:text-md" aria-haspopup="listbox" aria-expanded="true" aria-labelledby="listbox-label"
                                v-model="servicesId" @change="loadClinic">
                                    @foreach($service_data as $product_id => $product_name)
                                        <option class="block truncate" :value={{$product_id}}> {{$product_name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>

                        <div class="pt-6 pb-10">
                            <label id="listbox-label" class="block text-md font-medium font-bold text-gray-700"> {{ __('galaxy::app.customer.clinic.services.select-clinic')}} </label>
                            <div class="mt-1 relative mb-4">
                                <select type="select" class="bg-white relative w-full border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-green-800 focus:border-green-800 sm:text-md" aria-haspopup="listbox" aria-expanded="true" aria-labelledby="listbox-label" @change="loadtherapist" v-model="sellerId" >
                                    <option
                                        v-for="(item, index) in clinicId"
                                    :value="index">
                                        @{{item.name}}
                                    </option>
                                </select>
                            </div>
                        </div>

                        <div class="pt-6 pb-10">
                            <label id="listbox-label" class="block text-md font-medium text-gray-700 font-bold">{{ __('galaxy::app.customer.clinic.services.choose-therapist')}} </label>
                        
                            <div class="mt-1 relative">
                                <ul class="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-56 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-md" >
                                    
                                    <li class="text-gray-900 cursor-default relative py-2 pl-3 pr-9 wk_list" role="option" v-for="(item, index) in therapistID" :id="index" >
                                        <div class="flex items-center"  @click='therapist(index)' >
                                            <div class="w-15">
                                                <img v-bind:src="item.image">
                                            </div>
                                            <span class="font-normal ml-3 block truncate"> @{{item.name}} </span>
                                        </div>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    
                    <div class="border-t border-gray-200 bg-gray-50 grid grid-cols-1 divide-y divide-gray-200 sm:grid-cols-2 sm:divide-y-0 sm:divide-x mt-10 pb-10" @click= 'proceedSlot'>
                        <div class="px-6 py-5 text-md font-medium text-center bg-gray-800 hover:bg-gray-700" @click="backOneStep">
                            <span class="text-white font-bold">{{ __('galaxy::app.customer.clinic.therapist.back-one-step')}}</span>
                        </div>

                        <div class="px-6 py-5 text-md font-medium text-center bg-green-800 hover:bg-green-700"  >
                            <span class="text-white font-bold">{{ __('galaxy::app.customer.clinic.services.continue-time-selection')}}</span>
                        </div>
                    </div>
                </div>
            </div>    
        </div>
    </script>

    <script>
    Vue.component('therapist-services', {
        template: '#therapist-services-template',

        data: function() {
            return {
                servicesId: "Select Services",
                clinicId:[],
                sellerId: "Select Clinic",
                therapistID:[],
                doctor: "Select Thereapist",
                formData:{}, 
            }
        },

        methods: {
            loadClinic() {
                servicesId = this.servicesId;
                axios.post('{{ route("shop.customer.clinic.therapist.clinics") }}', {servicesId})
                .then(response => {
                    if (response) {
                        this.clinicId = response.data;
                    }
                }).catch(error => {
                    this.errorMessage = error.response.data.message;

                    console.log(this.errorMessage);
                });
                
            },
            loadtherapist($event){
                sellerId = this.sellerId

                axios.post("{{ route('shop.customer.clinic.therapist.therapist') }}", {sellerId})
                .then(response => {
                    if (response) {
                        this.therapistID = response.data;
                    }
                }).catch(error => {
                    this.errorMessage = error.response.data.message;
                });
            },
            therapist(id) {
                doctor = this.doctor;
                doctor = id;
                $(".wk_list").hide()
                $("#"+id).show()

            },
            proceedSlot() {
                formData = this.formData;
                formData.service_id = servicesId;
                formData.clinic_id = sellerId;
                formData.therapist_id = doctor;

                axios.post("{{ route('shop.customer.clinic.therapist.proceedSlot') }}", {formData})
                .then(response => {
                    if (response) {
                        window.location.href = "{{ route('shop.customer.clinic.therapist.slots')}}"
                    }
                }).catch(error => {
                    this.errorMessage = error.response.data.message;
                });
            },
            backOneStep() {
                window.location.href = "{{ route('shop.customer.clinic.therapist.company')}}"
            }
        }
    });
    </script>

@endpush