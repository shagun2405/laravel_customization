...
@section('content')
    ...
    <div class="therapist-panel" style="width:-webkit-fill-available">
        <div class="form-container" style="text-align: left">
            <h1 class="company_title">{{ __('galaxy::app.therapist.galaxy.title') }}</h1>

            <form method="POST" action="{{ route('shop.therapist.signin.verify') }}" @submit.prevent="onSubmit">
                @csrf
                ...
                <div class="button-group">
                    <button class="btn btn-xl btn-primary-galaxy">{{ __('admin::app.users.sessions.submit-btn-title') }}</button>
                </div>
            </form>
        </div>     
    </div>
