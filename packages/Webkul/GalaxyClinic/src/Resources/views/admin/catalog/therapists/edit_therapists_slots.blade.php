...
@section('page_title')
    {{ __('galaxyclinic::app.catalog.services.edit-clinics-slots') }}
@stop
...
@section('content')
    <div class="content">
        <form method="POST" action="{{ route('admin.galaxyclinic.catalog.therapists.updatetherapistsslots', $therapists_id) }}" @submit.prevent="onSubmit" enctype="multipart/form-data">
            <div class="page-header">
                ...
                <div class="page-action">
                    <button type="submit" class="btn btn-lg btn-primary">
                        {{ __('galaxyclinic::app.catalog.services.update-btn-title') }}
                    </button>
                </div>
            </div>
...
@push('scripts')
    <script type="text/x-template" id="appointment-booking-template">
        <div>
            ...
            <div class="control-group" :class="[errors.has('services') ? 'has-error' : '']">
                <label for="services">{{ __('galaxyclinic::app.catalog.therapists.services') }}</label>
                <select class="control" id="services" :name="'services['+clinicid+'][services][]'" :clinicid="clinicid" data-vv-as="&quot;{{ __('galaxyclinic::app.catalog.therapists.default_clinic') }}&quot;" multiple @change="get_services(clinicid)">
                    <option>click here to load services
                    </option>
                    <option v-for="(data,index) in service_data" :value="index">@{{data[0].name}}</option>
                </select>
                <span class="control-error" v-if="errors.has('services')">@{{ errors.first('services') }}</span>
            </div>
    ...
    <script>
        Vue.component('appointment-booking', {
            ...
            methods: {
                get_services(clinic_id){
                    axios.post('{{ route('admin.galaxyclinic.catalog.therapists.services') }}', {clinic_id})
                    .then(response => {
                        if (response.data) {
                        this.service_data = response.data;
                        } else {
...
<script type="text/x-template" id="service-slot-template">
    <tr>
        <td>    
            <label>Selected Services : </label> <input type="text" class="services" :value=service_name :v-placeholder=service_name style="border: transparent;" disabled></span>
        </td>
    </tr>
</script>

<script>
    let bookingProduct = @json($TherapistsSlotsDetails);
    Vue.component('slot-list', {

        template: '#slot-list-template',

        props: ['bookingType', 'sameSlotAllDays', 'clinicid','bookingProduct'],
        ...
        created: function() {
            let clicn = this.clinicid;
            var filteredArray = bookingProduct.filter(function(itm){
                return itm.clinic_id == clicn
            });
            
            this.slots['different_for_week'] = JSON.parse(filteredArray[0].slots);
            
            console.log(this.slots['different_for_week']);
        },
    ...
    Vue.component('service-slot', {

        template: '#service-slot-template',
        ...
        created:function() {
            ...
            service_name           = services[clinicid]
